...
TEST(HW1Test, FAST_NORMAL_DTERMINANT) {
    // test case
    for (int dimension = 0; dimension <= 10; dimension ++) {
        Matrix matrix{algebra::random(dimension, dimension, 1, 10)};
        EXPECT_NEAR(algebra::fast_determinant(matrix), algebra::determinant(matrix), 1e-6);
    }
